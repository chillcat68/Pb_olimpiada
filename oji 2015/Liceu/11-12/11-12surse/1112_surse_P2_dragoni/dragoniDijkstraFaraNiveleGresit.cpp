...
void dijkstra()
{
    ...
    while(!h.empty())
    {
        ...
        int dragon = (nod-1)/n+1;
        nod = (nod-1)%n+1;
        if(dmax[dragon]<dmax[nod])
            dragon=nod;
        ...
        {
            fiu = getNode(dragon, v[nod][i]);
            if(dist[v[nod][i]]>cdist+w[nod][i] && w[nod][i]<=dmax[dragon])
            {
                dist[v[nod][i]]=cdist+w[nod][i];
                h.push_back(make_pair(-dist[v[nod][i]], fiu));
